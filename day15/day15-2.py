...
for step in steps:
    ...
    if "=" in step:
        ...
        if iExistingIndex != -1:
            iNext = sBox.find("[",iExistingIndex + 1)
            debug(f"\t{iNext}")
            sBox = sBox[0:iExistingIndex] \
                    + "[" + label + ":" + focalLength + "]" \
                    + (sBox[iNext::] if iNext != -1 else "")
        else:
            if len(sBox) > 0:
        ...
        if iExistingIndex != -1:
            iNext = sBox.find("[",iExistingIndex + 1)
            debug(f"\t{iNext}")
            sBox = sBox[0:iExistingIndex] + (sBox[iNext::] if iNext != -1 else "")
            boxes[iBox] = sBox
    else:
