...
def getAdjacentTilesIfInBounds(c: Coord) -> list[Tile]:
  r = [
    tileDict[getCoordIfInBounds(c.x, c.y - 1)],     # Up
    tileDict[getCoordIfInBounds(c.x + 1, c.y)],     # Right
    tileDict[getCoordIfInBounds(c.x, c.y + 1)],     # Down
    tileDict[getCoordIfInBounds(c.x - 1, c.y)],     # Left
    tileDict[getCoordIfInBounds(c.x - 1, c.y - 1)], # Diagonal Up-Left
    tileDict[getCoordIfInBounds(c.x + 1, c.y - 1)], # Diagonal Up-Right
    tileDict[getCoordIfInBounds(c.x + 1, c.y + 1)], # Diagonal Down-Right
    tileDict[getCoordIfInBounds(c.x - 1, c.y + 1)], # Diagonal Down-Left
  ]
...
def getOutsideEdgeTiles() -> list(Tile):
  ...
  for x in range(nColumns):
    r.append(tileDict[x,0])
    r.append(tileDict[x,nRows - 1])
  for y in range(nRows): 
    r.append(tileDict[0,y])
    r.append(tileDict[nColumns - 1, y])
