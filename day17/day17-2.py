...
# file = open(f"./day{puzzleNumber}/day{puzzleNumber}_simple-example-input-2.txt",'r')
# file = open(f"./day{puzzleNumber}/day{puzzleNumber}_simple-example-input-3.txt",'r')
...
class PathNode:
    def __init__(self, x, y, prevDirection):
        self.x = x
        self.y = y
    ...
    def __eq__(self, other):
        return self.x == other.x and \
                self.y == other.y and \
                self.prevDirection == other.prevDirection
    ...
    def __str__(self):
        return f"({self.x}, {self.y}, {self.prevDirection})"
...
def dijkstra(adj_list, start):
    ...
    for node in adj_list:
        ...
        rtrn[PathNode(node[0], node[1], NORTH)] = (float('inf'), None)
        rtrn[PathNode(node[0], node[1], EAST)] = (float('inf'), None)
        rtrn[PathNode(node[0], node[1], SOUTH)] = (float('inf'), None)
        rtrn[PathNode(node[0], node[1], WEST)] = (float('inf'), None)

    # debug(list(map(lambda n: str(n), rtrn)))

    startNode = PathNode(start[0], start[1], UNKNOWN)
...
def sDijkstraReturn_pathOnly(d):
    ...
    def shortestAtCoords(coords, d):
        ...
        shortest = float('inf')
        for direction in [NORTH, EAST, SOUTH, WEST]:
            node = PathNode(coords[0], coords[1], direction)
...
endY = nRows - 1
for x in range(3):
    dN = d[PathNode(endX, endY, NORTH)][0]
    dE = d[PathNode(endX, endY, EAST)][0]
    dS = d[PathNode(endX, endY, SOUTH)][0]
    dW = d[PathNode(endX, endY, WEST)][0]
